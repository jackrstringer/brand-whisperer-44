...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const metricResults = allMetrics.map((metric) => {
      const name = metric.attributes?.name || "Unknown";
      const integration = metric.integration || metric.attributes?.integration || {};
...
async function fetchAllMetrics(apiKey: string): Promise<MetricWithIntegration[]> {
  ...
  while (url) {
    ...
    url = next ? (String(next).startsWith("http") ? String(next) : `${KLAVIYO_API_BASE}${next}`) : null;
...
async function fetchEventDataForMetric(apiKey: string, metricId: string) {
  try {
    const eventsUrl = new URL(`${KLAVIYO_API_BASE}/events`);
    eventsUrl.searchParams.set("filter", `equals(metric_id,\"${metricId}\")`);
    eventsUrl.searchParams.set("fields[event]", "event_properties");
    eventsUrl.searchParams.set("sort", "-datetime");

    const eventsResp = await fetch(eventsUrl.toString(), { headers: KLAVIYO_HEADERS(apiKey) });
