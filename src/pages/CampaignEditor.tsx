  ...
  const generateCampaign = async () => {
    ...
    const pollInterval = setInterval(async () => {
      ...
      if (data.status === "variants_ready" && data.variant_htmls) {
        ...
        // Check for product feed placeholder in any variant
        const allHtmls = [data.html, ...variants.map((v: any) => v.html)].filter(Boolean);
        if (allHtmls.some((h: string) => h.includes("PRODUCT_FEED_PLACEHOLDER"))) {
